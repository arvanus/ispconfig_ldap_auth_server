...
abstract class ISPConfig_SOAP extends Base
{
   ...
   */
  public function __construct($location, $uri, $user, $password, $validateCert)
  {
  ...
  protected function connectSoap()
  {
    ...
    $this->soap = new SoapClient(null, array(
      ...
      'uri' => $this->uri,
      $this->validateCert ?: 'stream_context' => stream_context_create(array('ssl' => array('verify_peer' => false, 'verify_peer_name' => false)))
    ));
   ...
   */
  public function getMailuserByMailbox($mailbox, $domain)
  {
    try {
      if ($this->session) {
   ...
   * @return bool|array false or Data Array containing all of the mailusers data fields as specified by ISPConfig mailuser api
   */
  public function getMailuserByLoginname($uid)
  {
    try {
      if ($this->session) {
  ...
  private function updateMailuser($mailuser, $newParams)
  {
    if (version_compare($this->getBackendVersion()['ispc_app_version'], '3.1dev', '<')) {
      $startdate = array(
        'year' => substr($mailuser['autoresponder_start_date'], 0, 4),
      ...
      $enddate = array(
        'year' => substr($mailuser['autoresponder_end_date'], 0, 4),
