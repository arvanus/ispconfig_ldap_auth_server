...
require_once __DIR__ . '/ispconfig_soap.php';
require_once __DIR__ . '/ispdomainuser.php';;
require_once __DIR__ . '/util.php';

//use ISPLDAP;
use ISPLDAP\lib\Util;
use ISPLDAP\lib\ISPDomainUser;
 ...
 */
class OC_User_ISPCONFIG extends ISPLDAP\lib\ISPConfig_SOAP
{
  /**
  ...
  {
    parent::__construct($location, $uri, $user, $password, array_key_exists('validateCert', $options) ? $options['validateCert'] : true);
  ...
  public function checkPassword($uid, $password)
  {
    if (!class_exists("SoapClient")) {
      ...
      Util::writeLog('user_ispconfig', 'ERROR: PHP soap extension is not installed or not enabled', Util::ERROR);
      return false;
    }
   ...
   */
  public function loginWithUIDFromIspc($uid, $password)
  {
    // get logins to check against the soap api
    $authResult = false;
  ...
  /**
   * Try to load user data from ISPConfig by its email
   *
   * @param string $uid
   * @param string $password
   * @return ISPDomainUser|bool Authenticated domain user or false
   */
  public function userDataWithUIDFromIspc($uid)
  {
    // get logins to check against the soap api
    $authResult = false;

    $this->connectSoap();
    if ($domainUser = $this->getMailuserByLoginname($uid)) {
      $authResult = $domainUser;
    }
    $this->disconnectSoap();
  ...
  private function getPossibleLogins($uid)
  {
    ...
    if ($returningUser) {
      /** @noinspection PhpDeprecationInspection */
      // Util::writeLog('user_ispconfig', "Found returning user: $returningUser", Util::DEBUG);
  ...
  private function tryDomainLoginWithMappedUID($user, $password)
  {
    ...
    if (is_array($mailuser) && count($mailuser)) {
      ...
      /** @noinspection PhpDeprecationInspection */
      Util::writeLog('user_ispconfig', "Login result for $user: $result", Util::DEBUG);
      return $result;
    }
    return false;
  }
  ...
  private function tryDomainLoginWithIspcUID($uid, $password)
  {
    ...
    if (count($mailuser)) {
      ...
      /** @noinspection PhpDeprecationInspection */
      Util::writeLog('user_ispconfig', "Login result for $uid: $domainuser", Util::DEBUG);
      return $domainuser;
    }
    return false;
  }
  ...
  {
    if (
      array_key_exists('domain_config', $this->options) &&
